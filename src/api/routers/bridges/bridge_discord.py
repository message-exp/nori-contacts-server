...
async def login_with_qr(user_id: str = "@barney:tomoron.com"):
...
async def logout(user_id: str = "@barney:tomoron.com"):
    data , status = await bridge_discord_service.logout(user_id)
    if status is not 200:
        raise HTTPException(status_code=status, detail=data)
    return data
@router.get("/ping" , response_model = PingResponse)
async def ping(user_id : str = "@barney:tomoron.com"):
    data , status = await bridge_discord_service.ping(user_id)
    if status is not 200:
        raise HTTPException(status_code=status, detail=data)
    return data
@router.post("/login/token" , response_model = LoginWithTokenResponse)
async def login_with_token(loginWithTokenRequest : LoginWithTokenRequest , user_id: str = "@barney:tomoron.com"):
