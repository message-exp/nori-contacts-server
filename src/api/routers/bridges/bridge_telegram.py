...
from schemas.bridge_telegram import (
    LoginRequest,
    CodeRequest,
    UserInfoResponse,
    MessageResponse,
...
async def login_user(
    login_request: LoginRequest, user_id: str = Depends(get_user_id_from_header)
):
    """
...
async def send_verification_code(
    code_request: CodeRequest, user_id: str = Depends(get_user_id_from_header)
):
    """
...
@router.post("/users/logout", response_model=MessageResponse)
async def logout_user(user_id: str = Depends(get_user_id_from_header)):
    ...
    if status != 200:
        raise HTTPException(status_code=status, detail=data)
    return MessageResponse(message="success logout")
